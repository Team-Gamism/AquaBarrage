...
public class Shark_Fin_Twin : Fin
{
    Vector3 startPos;
    ...
    void Shoot()
    {
        ...
        q = Quaternion.Euler(0f, 0f, q.eulerAngles.z);
        transform.DORotateQuaternion(q, 0.5f).onComplete += () => { speed = 7; };
        StartCoroutine(TwinShoot());
    }

    IEnumerator TwinShoot()
    {
        ...
        q = Quaternion.Euler(0f, 0f, q.eulerAngles.z);
        transform.DORotateQuaternion(q, 0.5f).onComplete += () => { speed = 13; };
    }
}
